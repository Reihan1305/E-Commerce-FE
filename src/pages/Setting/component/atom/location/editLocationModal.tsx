...
const EditLocationModal =() => {
  ...
  useEffect(() => {
    setPostalCode([]);
    if (selectedOption) {
      const matchedPostalCodes = PostalCode.filter(
        (op) => op.nama.toLowerCase() === selectedOption.value.toLowerCase()
      ).map((op) => op.kodePost);
      setPostalCode(matchedPostalCodes);
      setMarkerPosition([
        selectedOption.locationDetail.latitude,
        selectedOption.locationDetail.longitude,
      ]);
    } else {
    ...
    >
      <Box sx={style}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField {...params} label="Kota / Kecamatan" />
          )}
